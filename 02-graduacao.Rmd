...
```{r}
# gráfico de pizza do total por turno
```

## Alunos regulares ativos registrados nos cursos de graduação, por unidade acadêmica e sexo, UnB, 2019
```{r}
# tabela 17
...
tab6 %>% 
  tabela_geral(
    ...
    colunas = c("Unidade", "Fem 1ºS", "Masc 1ºS", "Total 1ºS", "Fem 2ºS", "Masc 2ºS", "Total 2ºS"),
    fundo = fundo
  ) %>% 
  footnote(general_title = "Fonte: Censo da Educação Superior 2019", general = "")
```
...
```{r}
# graf barra do total por semestre e sexo
```

<br>

## Alunos regulares registrados ativos e com trancamento geral de matrícula nos cursos de graduação, por unidade acadêmica, curso, habilitação e sexo, UnB, 2019
```{r}
# tabela 18
...
tab7 %>% 
  tabela_geral(
    titulo = "Alunos regulares registrados ativos e com trancamento geral de matrícula nos cursos de graduação, por unidade acadêmica, curso, habilitação e sexo, UnB, 2019",
    colunas = c("UNIDADE ACADÊMICA / CURSO / HABILITAÇÃO",
                "FEM 1ºS", "MASC 1ºS", "FEM 2ºS", "MASC 2ºS",
                "FEM 1ºS", "MASC 1ºS", "FEM 2ºS", "MASC 2ºS",
                "FEM 1ºS", "MASC 1ºS", "FEM 2ºS", "MASC 2ºS"),
    fundo = fundo,
    tamfonte = 12
...
```{r}
# graf de colunas com o percentual de matriculados por faixa etária
```
...
## Alunos estrangeiros regulares registrados nos cursos de graduação, por continente, país e unidade acadêmica, UnB, 2019 (2º semestre)
```{r}
# tabela 26
...
tab15 %>% 
  tabela_geral(
    titulo = "Alunos estrangeiros regulares registrados nos cursos de graduação, por continente, país e unidade acadêmica, UnB, 2019 (2º semestre)",
    fundo = fundo,
    tamfonte = 10
  ) %>% 
  row_spec(cor, background = alpha(fundo, 0.2)) %>% 
  # row_spec(0, background = fundo) %>% 
  footnote(general_title = "Fonte: Censo da Educação Superior 2019", 
           general = "")
```
...
```{r}
# graf de barras da evolução do total de matriculados por ano 
```
...
```{r}
# graf de barras da evolução do total de formados por ano 

# graf da evolução do número de formados por década
```
